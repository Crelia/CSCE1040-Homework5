...
void borrows::delborrow(int id, float fines)
{
	...
	if(difftime(time(&now), temp->getdate()) > 1209600)
	{
		...
		cout << "Fines must be paid before you can check the item(s) in. Patron owes $";
    ...
    while (temp != tail)
    {
        if (temp->getid() == id)
        {
        	if(difftime(time(&now), temp->getdate()) > 1209600)
        	{
	                ...
	                cout << "Fines must be paid before you can check item in. Patron owes " << temp->getfines() << endl;
        	        return;
       		}
...
void borrows::prtlist(int id)
{
	...
	if (temp->getpatid() == id)
	{
        	cout<< "Borrow id: " << temp->getid() <<" Patron: "<< temp->getpatid() << " Item: " << temp->getitemid() << endl;
        	temp = temp2;
	}
    }
    return;
...
void borrows::prtfines()
{
	...
	while (temp != NULL)
	{
		...
		if (temp->getfines() > 0)
		{
			cout << "Borrow id: " << temp->getid() << " Patron: " << temp->getpatid() << " Item: " << temp->getitemid() << "Fines: $" << temp->getfines() << endl << endl;
			temp = temp2;
		}
	}
	return;
...
void borrows::prtoverdues()
{
	...
	while ( temp != NULL)
	{
		...
		if (difftime(time(&now), temp->getdate()) > 1209600)
		{
			cout << "Borrow id: " << temp->getid() << " Patron: " << temp->getpatid() << " Item: " << temp->getitemid() << "Days overdue: " << (((temp->getdate()/60)/60)/24) - 14 << endl << endl;
			temp = temp2;
		}
	}
	return;
